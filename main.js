...
const menuItems = [...document.getElementsByClassName("nav-item"), document.getElementById('logo')];
...
[...menuItems].forEach(item=>{
    item.addEventListener("click", function(){
        document.getElementsByClassName('nav-item-active')[0].classList.remove('nav-item-active');
        (item.classList.contains('logo')) ? document.getElementById('home').classList.add('nav-item-active') : item.classList.add('nav-item-active');
        onNavigate(`/${item.id}`);
        onToggleMenu();
    });
})
...
const configureNavMenuBasedOnWidth = () => {
    if(width >= 992){
        ...
        if(!menuBackdrop.classList.contains('d-none')) menuBackdrop.classList.add('d-none');
    }else{
        if(!menu.classList.contains('col-6')){
            menu.classList.toggle('col-6');
        }
